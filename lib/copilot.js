...
(function () {
  ...
  // ── Recorder BroadcastChannel ─────────────────────────────────────
  let _recCh;
  try { _recCh = new BroadcastChannel('physiq-recorder'); } catch (_) {}

  function _sendRecCmd(cmd) { try { _recCh?.postMessage({ cmd }); } catch (_) {} }
  ...
  function _syncTab() {
    const tab   = TAB();
    const dot   = $('copilot-tab-dot');
    const badge = $('copilot-tab-badge');
    if (!tab) return;
    tab.classList.toggle('cop-tab-active',    _open);
    tab.classList.toggle('cop-tab-listening', _listening);
