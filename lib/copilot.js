...
(function () {
  ...
  async function _startStream() {
    ...
    const params = new URLSearchParams({ encoding: 'linear16', sample_rate: sampleRate, channels: 1 });
    ...
    _ws.addEventListener('open', async () => {
      ...
      // Prefer AudioWorklet (reliable on mobile); fall back to ScriptProcessor
      let wired = false;
      if (_audioCtx.audioWorklet) {
        try {
          ...
          _workletNode.connect(_audioCtx.destination); // keep graph active so process() fires
          wired = true;
        } catch (e) { console.warn('AudioWorklet failed, using ScriptProcessor fallback:', e); }
      }
      if (!wired) {
  ...
  function _stopStream() {
    ...
    // Flush remaining words before closing
