...
(function () {
  ...
  async function _startStream() {
    ...
    // Always use linear16 PCM — MediaRecorder produces WebM container which Deepgram
    // streaming cannot decode with encoding=opus (that expects raw Opus frames, not WebM)
    const mrMime = null;

    _audioCtx = new (window.AudioContext || window.webkitAudioContext)();
    if (_audioCtx.state === 'suspended') await _audioCtx.resume();
    const params = new URLSearchParams({ encoding: 'linear16', sample_rate: _audioCtx.sampleRate, channels: 1 });
