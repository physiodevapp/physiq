...
(function () {
  ...
  let _resumeRecAfterDictation     = false;  // paused the audio recorder for dictation?
  ...
  // ── Modo de ejecución ─────────────────────────────────────────────
  // El worker anuncia el modo en cada respuesta (X-PhysiQ-Mode). Aquí solo se
  // lee: el cliente nunca lo envía ni lo elige. _physiqSetMode (index.html)
  // reacciona si la sesión cae de real a demo a mitad de camino.
  const _noteMode = resp => {
    const m = resp.headers.get('X-PhysiQ-Mode');
    if (m) window._physiqSetMode?.(m);
    return m;
  };
  const _isDemo = () => {
    const m = window.PHYSIQ_MODE?.mode;
    return m === 'demo' || m === 'mixed';
  };
  ...
  function _show() {
    ...
    _syncTab();
    _syncHeaderChatActions();   // reopening on Consultar keeps New/History visible
  }
  ...
  async function _flushSuggest() {
    ...
    try {
      ...
      // 429: el motor pasivo dispara solo, así que un aviso por tarjeta sería
      // ruido. Se calla y reintentará con el siguiente fragmento.
      if (!resp.ok) return;
  ...
  // ── Region filter ────────────────────────────────────────────────────
  // recorder.js and peer.js create these stores lazily — only once something is
  // actually recorded or a peer session is written. Whoever opens the DB first
  // has to create them, or that first opener ends up with an empty 'physiq' v3
  // and every transaction throws NotFoundError. Since demo mode lets visitors
  // reach the copilot without ever recording, the copilot is now routinely
  // first. Same upgrade block as the other two openers.
  function _copUpgrade(e) {
  ...
  function _copOpenDB() {
    return new Promise((res, rej) => {
      ...
      req.onupgradeneeded = _copUpgrade;
  ...
  // iOS-style bounded swipe: the front follows the finger until the floating
  // red button is fully uncovered (SWIPE_OPEN), then rubber-bands with damped
  // travel instead of sliding off into empty space. A pull past SWIPE_COMMIT
  // arms the delete/dismiss on release. Shared by suggestion cards and history
  // rows.
  const SWIPE_OPEN   = 76;    // px of front travel that fully reveals the button
  const SWIPE_RESIST = 0.28;  // elastic factor applied to travel beyond SWIPE_OPEN
  const SWIPE_COMMIT = 116;   // pull distance that arms the action on release
  function _swipeTranslate(raw) {
    return raw <= SWIPE_OPEN ? raw : SWIPE_OPEN + (raw - SWIPE_OPEN) * SWIPE_RESIST;
  }

  // Progressive reveal: as the front slides left, ramp the trash strip's
  // intensity (bg alpha + icon scale) from base to full, hitting full exactly
  // at the commit threshold so the user feels it "arm". A CSS transition on the
  // strip smooths the follow and springs it back on release.
  function _rampReveal(bg, dx, width, frac) {
    if (!bg) return;
    const p = width ? Math.min(1, Math.abs(dx) / (width * frac)) : 0;
    bg.style.background = `rgba(239,68,68,${(0.9 + p * 0.1).toFixed(3)})`;
    const icon = bg.firstElementChild;
    if (icon) icon.style.transform = `scale(${(1 + p * 0.4).toFixed(3)})`;
  }

  function _initCardSwipe(card) {
    const front = card.querySelector('.cop-card-front');
    if (!front) return;
    const trashBg = card.querySelector('.cop-card-trash-bg');
    let startX = 0, startY = 0, raw = 0, axis = null, active = false;

    card.addEventListener('touchstart', e => {
      startX = e.touches[0].clientX;
      startY = e.touches[0].clientY;
      raw = 0; axis = null; active = true;
    ...
    card.addEventListener('touchmove', e => {
      ...
      if (axis !== 'x') return;
      raw = Math.max(0, startX - cx);   // leftward pull magnitude
      front.style.transform = `translateX(${-_swipeTranslate(raw)}px)`;
      _rampReveal(trashBg, -raw, SWIPE_COMMIT, 1);
    }, { passive: true });
    ...
    const onRelease = () => {
      ...
      if (raw >= SWIPE_COMMIT) {
        ...
        _rampReveal(trashBg, 0, SWIPE_COMMIT, 1);   // spring the reveal back
      }
    };
  ...
  function copilotSwitchTab(tab) {
    ...
    // New/History live in the header now — only relevant on Consultar.
    _syncHeaderChatActions();
  ...
  // The header's New/History buttons belong to the chat tab only. Mirror their
  // visibility to whether the chat panel is the active one.
  function _syncHeaderChatActions() {
    const act  = $('cop-hdr-chat-actions');
    const chat = $('cop-panel-chat');
    if (act && chat) act.hidden = chat.hidden;
  }
  ...
  function copilotGenerateSOAP() {
    ...
    fetch(`${WORKER_URL}/notes`, {
      ...
      .then(r => {
        _noteMode(r);
        if (r.status === 401) { window._actRevoke?.(); return null; }
        if (r.status === 429) { copilotAddSuggestion('test', 'Has alcanzado el límite de peticiones. Inténtalo de nuevo en un minuto.'); return null; }
  ...
  function copilotChatAutosize(el) {
    ...
    // Only allow scrolling once the box is capped at its max-height; below that,
    // keeping overflow hidden avoids a stray scrollbar line while the box grows.
    el.style.overflowY = sh > 96 ? 'auto' : 'hidden';
  }
  ...
  // The PhysiQ app icon (same artwork as favicon.svg), inlined so it can spin as
  // the "generating" indicator while a chat reply streams in. The gradient id is
  // namespaced (`copSpin`) to avoid colliding with the page favicon's `#g`.
  function _makeChatSpinner() {
    const el = document.createElement('span');
    el.className = 'cop-chat-spinner';
    el.setAttribute('aria-hidden', 'true');
    el.innerHTML =
      '<svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">' +
        '<defs><linearGradient id="copSpin" x1="0" y1="0" x2="1" y2="1">' +
          '<stop offset="0%" stop-color="#4f9cf9"/><stop offset="100%" stop-color="#38d9a9"/>' +
        '</linearGradient></defs>' +
        '<line x1="16" y1="10.5" x2="16" y2="6" stroke="url(#copSpin)" stroke-width="1.5" stroke-linecap="round"/>' +
        '<line x1="20.76" y1="13.25" x2="24.66" y2="11" stroke="url(#copSpin)" stroke-width="1.5" stroke-linecap="round"/>' +
        '<line x1="20.76" y1="18.75" x2="24.66" y2="21" stroke="url(#copSpin)" stroke-width="1.5" stroke-linecap="round"/>' +
        '<line x1="16" y1="21.5" x2="16" y2="26" stroke="url(#copSpin)" stroke-width="1.5" stroke-linecap="round"/>' +
        '<line x1="11.24" y1="18.75" x2="7.34" y2="21" stroke="url(#copSpin)" stroke-width="1.5" stroke-linecap="round"/>' +
        '<line x1="11.24" y1="13.25" x2="7.34" y2="11" stroke="url(#copSpin)" stroke-width="1.5" stroke-linecap="round"/>' +
        '<circle cx="16" cy="4.5" r="1.8" fill="#4f9cf9"/>' +
        '<circle cx="25.96" cy="10.25" r="1.8" fill="url(#copSpin)"/>' +
        '<circle cx="25.96" cy="21.75" r="1.8" fill="url(#copSpin)"/>' +
        '<circle cx="16" cy="27.5" r="1.8" fill="#38d9a9"/>' +
        '<circle cx="6.04" cy="21.75" r="1.8" fill="url(#copSpin)"/>' +
        '<circle cx="6.04" cy="10.25" r="1.8" fill="url(#copSpin)"/>' +
        '<circle cx="16" cy="16" r="4" fill="url(#copSpin)"/>' +
      '</svg>';
    return el;
  }
  ...
  async function copilotSendChat() {
    ...
    // Two-layer bubble while streaming: the markdown goes into `.cop-chat-stream`
    // (rewritten on every token) and the spinner is a sibling that is never
    // touched — so its rotation runs uninterrupted. `_endStream` flattens the
    // bubble back to the plain markdown representation used everywhere else
    // (history render, persisted messages), dropping the wrapper and spinner.
    ...
    if (bubble) {
      ...
      streamEl.className = 'cop-chat-stream';
      bubble.appendChild(streamEl);
      bubble.appendChild(_makeChatSpinner());
    }
    const _endStream = () => { if (bubble) bubble.innerHTML = _mdToHtml(answer); };
    ...
    try {
      ...
      if (resp.status === 429) {
        if (bubble) bubble.textContent = 'Has alcanzado el límite de peticiones. Inténtalo de nuevo en un minuto.';
        return;
      }
      ...
      while (true) {
        ...
        while ((nl = buffer.indexOf('\n')) !== -1) {
          ...
          if (evt.text) {
            answer += evt.text;
            if (streamEl) streamEl.innerHTML = _mdToHtml(answer);
            if (msgs) msgs.scrollTop = msgs.scrollHeight;
          } else if (evt.truncated) {
            truncated = true;   // reply hit the worker's token ceiling
          }
        }
      }
      ...
      // Flag a cut-off reply so the physio knows to ask for the rest ("continúa"
      // resends the thread, so Claude picks up where it left off). The notice is
      // UI-only — the stored/persisted message text stays clean.
      if (truncated && bubble) {
        const note = document.createElement('div');
        note.className = 'cop-chat-trunc';
        note.textContent = 'Respuesta recortada. Escribe «continúa» para seguir.';
        bubble.appendChild(note);
        if (msgs) msgs.scrollTop = msgs.scrollHeight;
      }
      // Marca de respuesta precargada, con el mismo criterio que el aviso de
      // recorte: se añade tras _endStream() (que reescribe el innerHTML de la
      // burbuja) y es solo de UI — el texto que se persiste queda limpio.
      if (_isDemo() && bubble) {
        const note = document.createElement('div');
        note.className = 'cop-chat-demo';
        note.textContent = 'Respuesta de ejemplo (modo demo) — no se ha llamado a ningún modelo.';
        bubble.appendChild(note);
        if (msgs) msgs.scrollTop = msgs.scrollHeight;
      }
  ...
  async function _histRenderList() {
    ...
    for (const it of items) {
      ...
      // Trash-red reveal behind the front (visual feedback while swiping).
      const trashBg = document.createElement('div');
      trashBg.className = 'cop-hist-trash-bg';
      trashBg.setAttribute('aria-hidden', 'true');
      trashBg.innerHTML =
      ...
      // Sliding front — holds the card content and (on pointer devices) the trash icon.
      const front = document.createElement('div');
      front.className = 'cop-hist-item-front';
      front.onclick = () => {
        if (row._histSwiping) return;                        // ignore the click that trails a drag
        copilotHistoryLoad(it.id);
      };
      ...
      del.onclick = e => { e.stopPropagation(); copilotHistoryDelete(it.id, it.title); };

      front.appendChild(meta);
      front.appendChild(del);
      row.appendChild(trashBg);
      row.appendChild(front);
      list.appendChild(row);
      _initHistSwipe(row, front, it.id, it.title);
    }
  }

  // Swipe-to-delete for history rows (touch). Dragging the front left past the
  // threshold springs it back and opens the confirm dialog — same gesture as the
  // suggestion cards, but a saved conversation is only removed after confirming.
  function _initHistSwipe(row, front, id, title) {
    const trashBg = row.querySelector('.cop-hist-trash-bg');
    let startX = 0, startY = 0, raw = 0, axis = null, active = false;
    ...
    const onRelease = () => {
      if (!active) return;
      active = false;
      // Always spring back; the dialog is the point of no return, not the swipe.
      front.style.transition = 'transform 0.22s ease';
      front.style.transform  = '';
      _rampReveal(trashBg, 0, SWIPE_COMMIT, 1);   // spring the reveal back
      if (axis === 'x' && raw >= SWIPE_COMMIT) {
        row._histSwiping = true;
        setTimeout(() => { row._histSwiping = false; }, 320);   // swallow the trailing click
        copilotHistoryDelete(id, title);
      }
    };

    row.addEventListener('touchend',    onRelease, { passive: true });
    row.addEventListener('touchcancel', onRelease, { passive: true });
  }
  ...
  // Ask before removing — reached from the swipe gesture or the trash icon.
  function copilotHistoryDelete(id, title) {
    const t = (title || '').trim();
    _copConfirm(
      'Borrar conversación',
      t ? `Se eliminará «${t}» del historial.` : 'Se eliminará esta consulta del historial.',
      () => _histDoDelete(id),
    );
  }
  ...
  function _copConfirm(title, sub, onOk) {
    const el = $('cop-confirm');
    if (!el) { onOk(); return; }   // no dialog in DOM — fail open
    const t = $('cop-confirm-title'), s = $('cop-confirm-sub');
    if (t) t.textContent = title;
    if (s) s.textContent = sub;
    _confirmAction = onOk;
    el.hidden = false;
  }
  function copilotConfirmCancel() {
    const el = $('cop-confirm'); if (el) el.hidden = true;
    _confirmAction = null;
  }
  function copilotConfirmOk() {
    const el = $('cop-confirm'); if (el) el.hidden = true;
    const a = _confirmAction; _confirmAction = null;
    if (a) a();
  }
  ...
  function copilotChatDictate() {
    ...
    // Also pause the audio recorder if it's rolling: what the physio dictates *to*
    // the assistant shouldn't leak into the consultation recording. Same rationale
    // as pausing the passive engine. Only resume what we paused ourselves.
    const recState = document.getElementById('rec-widget')?.dataset.state;
    _resumeRecAfterDictation = recState === 'recording';
    if (_resumeRecAfterDictation && typeof recCmd === 'function') recCmd('pause');
  ...
  function _finishDictation() {
    ...
    if (_resumeRecAfterDictation) {
      _resumeRecAfterDictation = false;
      // No-op unless the recorder is still paused (recCmd/_resume guards state),
      // so a user who stopped/discarded mid-dictation isn't overridden.
      if (typeof recCmd === 'function') recCmd('resume');
    }
  }
  ...
  try {
    ...
    ch.onmessage = e => {
      ...
      if (d.force          !== undefined) _sessionData.force          = d.force;
      if (d.questionnaires !== undefined) _sessionData.questionnaires = d.questionnaires;
      if (d.assessment     !== undefined) _sessionData.assessment     = d.assessment;
      if (d.balance        !== undefined) _sessionData.balance        = d.balance;
      // SESSION_JUMP carries the array under `jumps` (plural); IDB stores it as
      // `jump` (singular). Mirror peer.js: read `jumps`, store under `jump`.
      if (d.jumps          !== undefined) _sessionData.jump           = d.jumps;
      if (d.kinematics     !== undefined) _sessionData.kinematics     = d.kinematics;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      const req = indexedDB.open('physiq', 3);
      req.onupgradeneeded = _copUpgrade;   // see _copOpenDB — may be the first opener
      req.onsuccess = () => {
        const db  = req.result;
        ...
        get.onsuccess = () => {
          ...
          if (s) {
            ...
            if (s.force)          _sessionData.force          = s.force;
            if (s.questionnaires) _sessionData.questionnaires = s.questionnaires;
            if (s.assessment)     _sessionData.assessment     = s.assessment;
            if (s.balance)        _sessionData.balance        = s.balance;
            if (s.jump)           _sessionData.jump           = s.jump;
            if (s.kinematics)     _sessionData.kinematics     = s.kinematics;
            if (s.manualRegion) {
  ...
  window.copilotConfirmCancel     = copilotConfirmCancel;
  window.copilotConfirmOk         = copilotConfirmOk;
