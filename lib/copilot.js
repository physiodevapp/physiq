...
(function () {
  ...
  async function _startStream() {
    ...
    // Prefer MediaRecorder/Opus — native browser encoding, no manual PCM conversion
    const mrMime = (typeof MediaRecorder !== 'undefined' &&
      MediaRecorder.isTypeSupported('audio/webm;codecs=opus'))
      ? 'audio/webm;codecs=opus' : null;

    let params;
    if (mrMime) {
      params = new URLSearchParams({ encoding: 'opus', container: 'webm' });
    } else {
      _audioCtx = new (window.AudioContext || window.webkitAudioContext)();
      if (_audioCtx.state === 'suspended') await _audioCtx.resume();
      params = new URLSearchParams({ encoding: 'linear16', sample_rate: _audioCtx.sampleRate, channels: 1 });
    }
    ...
    _ws.addEventListener('open', async () => {
      ...
      if (mrMime) {
        // ── MediaRecorder path (preferred: browser handles encoding) ──
        _mediaRecorder = new MediaRecorder(_audioStream, { mimeType: mrMime });
        _mediaRecorder.ondataavailable = e => {
          if (e.data.size > 0 && _ws?.readyState === WebSocket.OPEN) {
            e.data.arrayBuffer().then(buf => {
              _ws.send(buf);
              _pktCount++;
              if (_pktCount % 4 === 0) _setStatus('listening', `Escuchando… ▶ ${_pktCount}`);
            });
          }
        };
        _mediaRecorder.start(250);
      } else {
        // ── PCM fallback: AudioWorklet → ScriptProcessor ──
        ...
        const sendPCM = f32 => {
          ...
          _pktCount++;
          if (_pktCount % 20 === 0) {
            const lvl = Math.round(_peakLevel * 100);
            _peakLevel = 0;
            _setStatus('listening', `Escuchando… ▶ ${_pktCount} mic:${lvl}%`);
          }
        };
    ...
    _ws.addEventListener('message', e => {
      ...
      console.log('DG msg:', msg.type, msg.is_final ?? '', (msg.channel?.alternatives?.[0]?.transcript || '').slice(0, 80));
      _onDgMessage(msg);
    });
  ...
  function _onDgMessage(msg) {
    if (msg.type === 'Results') {
      ...
      if (!text) return;
      if (!msg.is_final) {
        // Show live partial recognition in status so mobile users can see it without DevTools
        _setStatus('listening', `◉ ${text.length > 50 ? text.slice(-50) : text}`);
        return;
      }
      _setStatus('listening', `Escuchando… ▶ ${_pktCount}`);
  ...
  async function _flushSuggest() {
    ...
    try {
      const resp = await fetch(`${WORKER_URL}/suggest`, {
        ...
        body: JSON.stringify({ query, session: _sessionData }),
      });
      if (!resp.ok) return;
      const { type, text } = await resp.json();
