...
(function () {
  ...
  // at the commit threshold so the user feels it "arm". Shared by the
  // suggestion cards and the history rows; a CSS transition on the strip
  // smooths the follow and springs it back on release.
  ...
  function _initCardSwipe(card) {
    ...
    card.addEventListener('touchmove', e => {
      ...
      _rampReveal(trashBg, dx, card.offsetWidth, 0.45);
    }, { passive: true });
    ...
    const onRelease = () => {
      ...
      if (dx < -(card.offsetWidth * 0.45)) {
        ...
        _rampReveal(trashBg, 0, card.offsetWidth, 0.45);   // spring the reveal back
      }
    };
  ...
  function _initHistSwipe(row, front, id, title) {
    ...
    row.addEventListener('touchmove', e => {
      ...
      if (!axis) axis = Math.abs(cx - startX) > Math.abs(cy - startY) ? 'x' : 'y';
      if (axis !== 'x') return;
      dx = Math.min(0, cx - startX);   // leftward only
      front.style.transform = `translateX(${dx}px)`;
      _rampReveal(trashBg, dx, row.offsetWidth, 0.4);
    }, { passive: true });
    ...
    const onRelease = () => {
      ...
      front.style.transform  = '';
      _rampReveal(trashBg, 0, row.offsetWidth, 0.4);   // spring the reveal back
      if (axis === 'x' && dx < -(row.offsetWidth * 0.4)) {
