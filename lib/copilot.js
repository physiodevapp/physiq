...
(function () {
  ...
  async function _startStream() {
    ...
    console.log('[cop] ctx state=', _audioCtx.state, 'sr=', _audioCtx.sampleRate);
    ...
    const sendPCM = f32 => {
      ...
      _pktCount++;
      if (_pktCount === 1) {
        console.log('[cop] first PCM pkt: samples=', i16.length, 'bytes=', i16.buffer.byteLength, 'peak=', peak.toFixed(4));
      }
      if (_pktCount % 20 === 0) {
    ...
    if (_audioCtx.audioWorklet) {
      try {
        ...
        wired = true;
        console.log('[cop] AudioWorklet wired');
      } catch (e) { console.warn('[cop] AudioWorklet failed, using ScriptProcessor:', e); }
    }
    if (!wired) {
      ...
      console.log('[cop] ScriptProcessor wired');
    }
