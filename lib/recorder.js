...
// Cache DOM elements — script is at end of <body>, DOM is ready
const _recWidget = document.getElementById('rec-widget');
const _timerMM   = document.querySelector('.rw-timer-mm');
const _timerSS   = document.querySelector('.rw-timer-ss');
...
async function _start(fromPeer = false) {
  ...
  try {
    ...
    _recWidget.dataset.error = '1';
    setTimeout(() => delete _recWidget.dataset.error, 2500);
  }
}
...
function _setTimer(s) {
  _timerMM.textContent = String(Math.floor(s / 60)).padStart(2, '0');
  _timerSS.textContent = String(s % 60).padStart(2, '0');
}

function _updateWidget() {
  _recWidget.dataset.state = _recState;
  _setTimer(_duration);
  if (typeof _peerUpdateRecModeUI === 'function') _peerUpdateRecModeUI();
}

function showDiscardConfirm() {
  _recWidget.dataset.confirm = '1';
}

function hideDiscardConfirm() {
  delete _recWidget.dataset.confirm;
}
