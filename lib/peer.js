...
async function _peerApplySync(msg) {
  ...
  const remoteHasData = !!(msg.patient || msg.rom);
  if (!remoteHasData) return;

  const localHasData = !!(cur && (cur.patient || cur.rom));
  if (!localHasData) {
...
async function _peerApplySyncForce(session) {
  await _peerWriteSession({ patient: session.patient ?? '', rom: session.rom ?? null });
...
// fields needed to reconstruct it: ufrag, pwd, fingerprint, DTLS role, candidates.
// Binary layout: [0x01][ufrag_len][ufrag][pwd_len][pwd][32 fp][1 role][1 ncand][6×cand]
...
function _sdpToBytes(sdp) {
  ...
  if (!ufrag || !pwd || !fpLine || !role) return null;
  ...
  const pwdB   = new TextEncoder().encode(pwd);
  const roleB  = role === 'actpass' ? 0 : role === 'active' ? 1 : 2;

  const buf = new Uint8Array(1 + 1 + ufragB.length + 1 + pwdB.length + 32 + 1 + 1 + cands.length * 6);
  let i = 0;
  buf[i++] = 0x01;  // format version
  buf[i++] = ufragB.length; buf.set(ufragB, i); i += ufragB.length;
  buf[i++] = pwdB.length;   buf.set(pwdB,   i); i += pwdB.length;
  buf.set(fp, i); i += 32;
  buf[i++] = roleB;
  buf[i++] = cands.length;
...
function _bytesToSdp(buf) {
  ...
  const pwdLen   = buf[i++]; const pwd   = new TextDecoder().decode(buf.slice(i, i + pwdLen));   i += pwdLen;
