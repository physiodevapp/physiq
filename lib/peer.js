...
async function _peerApplySync(msg) {
  ...
  // Only apply if remote data is strictly newer than local
  if (cur && (cur.updatedAt ?? 0) >= (msg.updatedAt ?? 0)) return;
  await _peerWriteSession({ patient: msg.patient ?? '', rom: msg.rom ?? null });
  _peerSessCh.postMessage({ type: 'SESSION_PATIENT', patient: msg.patient ?? '',  _relay: true });
  _peerSessCh.postMessage({ type: 'SESSION_ROM',     rom:     msg.rom     ?? null, _relay: true });
}
...
if (typeof _peerInitHash !== 'undefined' && _peerInitHash) showPeerPanel();
