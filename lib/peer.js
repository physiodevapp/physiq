...
async function _peerApplySync(msg) {
  ...
  const remoteHasData = !!(msg.patient || msg.rom || msg.assessmentState);
  if (!remoteHasData) return;

  const localHasData = !!(cur && (cur.patient || cur.rom || cur.assessmentState));
  if (!localHasData) {
...
async function _peerApplySyncForce(session) {
  await _peerWriteSession({ patient: session.patient ?? '', rom: session.rom ?? null, assessmentState: session.assessmentState ?? null });
...
function _onPeerData(raw) {
  ...
  } else if (msg.type === 'SESSION_ASSESSMENT_STATE') {
    _peerWriteSession({ assessmentState: msg.assessmentState ?? null }).catch(console.warn);
  } else if (msg.type === 'SESSION_ASSESSMENT') {
    _peerWriteSession({ assessment: msg.assessment ?? null }).catch(console.warn);
