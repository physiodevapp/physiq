...
async function _peerSyncOnConnect() {
  ...
  try {
    peer.dc.send(JSON.stringify({
      ...
      questionnaires:  session.questionnaires  ?? null,
...
async function _peerApplySyncForce(session) {
  ...
  if (Array.isArray(session.jump) && session.jump.length) {
    _peerSessCh.postMessage({ type: 'SESSION_JUMP', jumps: session.jump, _relay: true });
  }
  if (session.balance && Object.keys(session.balance).length) {
    _peerSessCh.postMessage({ type: 'SESSION_BALANCE', balance: session.balance, _relay: true });
  }
  if (session.kinematics) {
    _peerSessCh.postMessage({ type: 'SESSION_KINEMATICS', kinematics: session.kinematics, _relay: true });
  }
  if (Array.isArray(session.questionnaires) && session.questionnaires.length) {
    _peerSessCh.postMessage({ type: 'SESSION_QUESTIONNAIRE', questionnaires: session.questionnaires, _relay: true });
  }
...
const _peerIDBHandlers = {
  ...
  SESSION_JUMP:             msg => _peerWriteSession({ jump:            msg.jumps           ?? null }),
  SESSION_BALANCE:          msg => _peerWriteSession({ balance:         msg.balance         ?? null }),
  SESSION_KINEMATICS:       msg => _peerWriteSession({ kinematics:      msg.kinematics      ?? null }),
  SESSION_QUESTIONNAIRE:    msg => _peerWriteSession({ questionnaires:  msg.questionnaires  ?? null }),
